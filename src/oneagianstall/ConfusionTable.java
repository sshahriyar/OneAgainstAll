...
public class ConfusionTable {
    ...
    public void updateCorrectPrediction(Integer rank, String correctClass){
       ...
       rank=rank-1;
       HashMap<String,HashMap<Metrics,Integer>> theRank=confusionTable.get(rank);
     ...
     */
    public void updateWrongPrediction(Integer rank,String correctClass, String predictedClass){
        
       correctClass=correctClass.trim();
       predictedClass=predictedClass.trim();
       rank=rank-1;
      // System.out.println("rank corr pred "+ rank + " "+correctClass+ " "+predictedClass);
      ...
      // theRank.put(correctClass, theClass);
       ...
       for (Map.Entry<String,HashMap<Metrics,Integer>> classes:theRank.entrySet()){
           String key=classes.getKey();
           if (!key.equalsIgnoreCase(correctClass) && !key.equalsIgnoreCase(predictedClass)){
              HashMap<Metrics,Integer> metric= classes.getValue();
              Integer tn=metric.get(Metrics.TN);
              metric.put(Metrics.TN, (tn+1));
           }
        ...
        for (Map.Entry<Integer, HashMap<String,HashMap<Metrics,Integer>>> ranks:
         ...
         // second get the classess
          ...
          for (Map.Entry<String,HashMap<Metrics,Integer>> classes:theRank.entrySet()){
               ...
               //Integer positivePredForClass=metric.get(Metrics.TP)+metric.get(Metrics.FP);
               Integer positivePredForClass=allPositivePredForClass[classCount]+metric.get(Metrics.FP);
               
               Double csmfPred=positivePredForClass.doubleValue()/totalRecords.doubleValue();
               
               allActualPositives[classCount]+=metric.get(Metrics.TP);
               //Integer actualPositivesForClass=metric.get(Metrics.TP)+metric.get(Metrics.FN);
               Integer actualPositivesForClass=allActualPositives[classCount]+ metric.get(Metrics.FN);
               
               Double csmfCorrect=actualPositivesForClass.doubleValue()/totalRecords.doubleValue();
               if (csmfMin==0.0 || csmfCorrect<csmfMin) {
                   csmfMin=csmfCorrect;
               }  
               classCount++;
              // if (csmfMin<=overallCsmfMin)
                //   overallCsmfMin=csmfMin;
               csmfNumerator=csmfNumerator+Math.abs(csmfCorrect-csmfPred);
          ...
          //Double specificity=tn.doubleValue()/(tn.doubleValue()+fp.doubleValue());
          //Double sensitivity=tp.doubleValue()/(tp.doubleValue()+fn.doubleValue());
         ...
         // System.out.println("sensitivity:"+sensitivity + ", specificity:"+specificity+ " "+(tp+fp)
           //       +" "+fn + " "+ fp+ " "+tn+ " csmf accuracy: "+ csmfAccuracy + " overallSens: "+sensitivityOverall
             //     + " overallSpec: "+specificityOverall );
         ...
         System.out.println( " sensitivity: "+sensitivityOverall
                  + " speficity: "+specificityOverall + " PCCC: "+PCCC+ " csmf accuracy: "+ csmfAccuracy );
